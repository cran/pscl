...
plot(clog(ofp) ~ health, data = dt, varwidth = TRUE,
  ylab = "Log-number of physician office visits", xlab = "Self-perceived health status", main = "health")
plot(clog(ofp) ~ cfac(numchron), data = dt,
  ylab = "Log-number of physician office visits", xlab = "Number of chronic conditions", main = "numchron")
plot(clog(ofp) ~ privins, data = dt, varwidth = TRUE,
  ylab = "Log-number of physician office visits", xlab = "Covered by private insurance", main = "privins")
plot(clog(ofp) ~ cfac(hosp, c(0:2, 8)), data = dt,
  ylab = "Log-number of physician office visits", xlab = "Number of hospital stays", main = "hosp")
plot(clog(ofp) ~ gender, data = dt, varwidth = TRUE,
  ylab = "Log-number of physician office visits", xlab = "Gender", main = "gender")
...
fm_summary[30,] <- format(sapply(fm, function(x) round(logLik(x), digits = 1)))
